...
def run_lftp(host, username, password, local_file, remote_path, local_file_ctrl, remote_path_ctrl, ti=None):
    ...
    EOF
    """
    print("lftp_command:",lftp_command)
    try:
...
def upload_csv_ctrl_to_ftp_server_v2(default_emails: Dict[str, List[str]], host_ftps_server: str, username_ftps_server: str, password_ftps_server: str, path_ftp: str, slack_webhook: Optional[str] = None, **kwargs):
    """
    Upload files to FTPS server using lftp with error handling and notifications.
    """
    try:
        ...
        # Prepare file paths
        csv_remote_path = f'{path_ftp}/{output_filename_csv}'
        ctrl_remote_path = f'{path_ftp}/{output_filename_ctrl}'

        remote_path = f'{path_ftp}'
        ...
        print(f"Starting upload of {csv_local_file_path} and {ctrl_local_file_path}...")
        run_lftp(
            host=host_ftps_server,
            username=username_ftps_server,
            password=password_ftps_server,
            local_file=csv_local_file_path,
            remote_path=csv_remote_path,
            local_file_ctrl=ctrl_local_file_path,
            remote_path_ctrl=ctrl_remote_path,
...
def upload_csv_ctrl_to_ftp_server_monthly_v2(default_emails: Dict[str, List[str]], host_ftps_server: str, username_ftps_server: str, password_ftps_server: str, path_ftp: str, slack_webhook: Optional[str] = None, **kwargs):
    """
    Upload files to FTPS server using lftp with error handling and notifications.
    """
    try:
...
def upload_csv_ctrl_to_ftp_server_manual_v2(default_emails: Dict[str, List[str]], host_ftps_server: str, username_ftps_server: str, password_ftps_server: str, path_ftp: str, slack_webhook: Optional[str] = None, **kwargs):
    """
    Upload files to FTPS server using lftp with error handling and notifications.
    """
    ...
    try:
        ...
        host_ftps_server = conf.get('HOST_FTPS_SERVER', host_ftps_server)
        username_ftps_server = conf.get('USERNAME_FTPS_SERVER', username_ftps_server)
        password_ftps_server = conf.get('PASSWORD_FTPS_SERVER', password_ftps_server)
        path_ftp = conf.get('PATH_FTP', path_ftp)
        ...
        if run_id_conf:
            raise AirflowSkipException(f"Skipping notification because get run_id: {run_id_conf}")
        # Check if upload should be skipped
        should_upload_ftp = ti.xcom_pull(key='should_upload_ftp', task_ids='validate_input_task_manual_v2')
        if not should_upload_ftp:
            raise AirflowSkipException("FTPS upload disabled in configuration")
