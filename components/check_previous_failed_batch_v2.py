...
def check_previous_failed_batch(**context):
    ...
    try:
        with create_session() as session:
            if run_id_conf:
                # กรองเฉพาะ DAG Run ที่มี run_id ตรงกับ run_id_conf
                ...
                print(f"Number of failed DAG runs for run_id {run_id_conf}: {len(failed_dag_runs)}")
                if not failed_dag_runs:
                    print(f"No failed DAG runs found for run_id: {run_id_conf}")
                    return
            else:
                # ถ้าไม่มี run_id ให้สร้าง log ว่าไม่มี run_id และข้ามการประมวลผล
                print("No run_id provided in the configuration. Skipping failed batch processing.")
                return
            ...
            for failed_dag_run in failed_dag_runs:
                ...
                # Update state of the failed DAG run to 'queued'
